 ...
 */
public class JSRReportGenerator {

    /** Default constructor. */
    public JSRReportGenerator() {
    }   
    ...
    public void generate(APIComparator comp, Comments existingComments) {
        String fullReportFileName = reportFileName  + ".sectionseven";
        if (outputDir != null)
            fullReportFileName = outputDir + JDiff.DIR_SEP + reportFileName;
        System.out.println("JDiff: generating HTML report into the file '" + fullReportFileName + ".txt" + "' and the subdirectory '" + fullReportFileName + "'");
        ...
        String sectionSixSummaryFileName = fullReportFileName + JDiff.DIR_SEP +
        reportFileName + "-sectionsix-summary" + ".txt";
        writeSectionSixSummary(comp, sectionSixSummaryFileName);
        
        String sectionSixReportFileName = fullReportFileName + JDiff.DIR_SEP +
        reportFileName + "-sectionsix" + ".txt";
        writeSectionSixReport(comp, sectionSixReportFileName);
        
        String sectionSevenReportFileName = fullReportFileName + JDiff.DIR_SEP +
                reportFileName + "-sectionseven" + ".txt"; 
        writeSectionSevenReport(comp, sectionSevenReportFileName);
        
        String newClassSpreadSheetFileName = fullReportFileName + JDiff.DIR_SEP +
                reportFileName + "-spreadsheet" + ".csv"; 
        writeNewClassSpreadSheet(comp, newClassSpreadSheetFileName);

    }   

    private void writeSectionSixSummary(APIComparator comp,
            String sectionSixSummaryFileName) {
        File reportFile = new File(sectionSixSummaryFileName);
        ...
        // basically, we need to dump the following information. 
        // the newly added package at the method level. 
        API newAPI = comp.getNewAPI();
        newAPI.dumpPackagesAtClassLevelBrief(writer);
        writer.close();
    }

    private void writeNewClassSpreadSheet(APIComparator comp,
            String newClassSpreadSheetFileName) {
        File reportFile = new File(newClassSpreadSheetFileName);
        ...
        // the newly added package at the method level. 
        API newAPI = comp.getNewAPI();
        newAPI.dumpPackagesAtClassLevelInCsvFormat(writer);
        writer.close();
    }

    private void writeSectionSixReport(APIComparator comp,
        ...
        // the newly added package at the method level. 
        API newAPI = comp.getNewAPI();
        newAPI.dumpPackagesAtMethodLevel(writer);
        writer.close();
    }
    
    private void writeSectionSevenReport(APIComparator comp,
            String fullReportFileName) {
        File reportFile = new File(fullReportFileName);
        ...
        // the newly added package at the method level. 
        API newAPI = comp.getNewAPI();
        newAPI.dumpPackagesAtClassLevel(writer);
        writer.close();
    }
